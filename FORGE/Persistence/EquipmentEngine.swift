...
extension AppState {
    ...
    func owns(_ group: EquipmentGroup) -> Bool {
        group == .noneNeeded || userData.ownedEquipment.contains(group)
    }
    ...
    func canPerform(_ template: ExerciseTemplate) -> Bool {
        template.needsNoEquipment || userData.ownedEquipment.contains(template.equipmentGroup)
    }
    ...
    func availableExerciseCount(for equipment: Set<EquipmentGroup>) -> Int {
        ExerciseLibrary.all.filter { $0.needsNoEquipment || equipment.contains($0.equipmentGroup) }.count
    }
